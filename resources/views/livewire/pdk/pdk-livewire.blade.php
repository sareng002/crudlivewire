<x-slot name='header'>
    <h1 class="font-semibold text-gray-700">Data Penduduk</h1>
</x-slot>
    ...
    <div class="mx-w-7 mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow:hidden shad-xl sm:rounded-lg px-4 py-4">
            ...
            <button wire:click="create()"
                class="bg-indigo-600 hover:bg-indigo-800 text-white px-2 py-2 rounded-md flex ">
            ...
            <div class="">
                <table class="table w-full table-compact mt-4">
                    <thead>
                        <tr class="">
                            ...
                            <th class="border   px-4 py-3 w-100">nik</th>
                            <th class="border   px-4 py-3">kk</th>
                            <th class="border   px-4 py-3">nama</th>
                            <th class="border   px-4 py-3 w-30">Alamat</th>
                            <th class="border   px-4 py-3 w-20">ACTION</th>

                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($pdkmodels as $row)
                        <tr>
                            <td class="border text-sm px-3 py-1">{{ $loop->iteration }}</td>
                            <td class="border text-sm px-3 py-1">{{ $row->nik }}</td>
                            <td class="border text-sm px-3 py-1">{{ $row->kk }}</td>
                            <td class="border text-sm px-3 py-1">{{ $row->nama }}</td>
                            <td class="border text-sm px-3 py-1">{{ $row->alamat }}</td>
                ...
                <p class="mt-4 text-sm text-red-500 ">{{ $pdkmodels->links() }}</p>
